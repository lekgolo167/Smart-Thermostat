...
int32_t sd1306_send_command(uint8_t addr, uint8_t cmd)
{
	...
	retval = Wire.write(data_to_send, 2);
	return retval;
}
...
int32_t sd1306_write_data(uint8_t addr, uint8_t *data)
{
	...
	// Send the data by I2C bus
	Wire.beginTransmission(addr);
	retval = Wire.write(data_to_send, 1025);
	return retval;
}
...
/**
  ...
  * @retval None.
  */
void sd1306_draw_string(int32_t x, int32_t y, char* textptr, int32_t size, uint8_t color)
{
	// Loop counters
	uint8_t i;
	uint8_t j;
