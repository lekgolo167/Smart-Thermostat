...
Thermostat::Thermostat(RTCZero& rtc) {
	m_rtc = rtc;
...
void Thermostat::update_cycle() {
	uint8_t today = m_rtc.getDay();
	cycle* loaded_cycle = m_days[today]->find_next_cycle(m_rtc.getHours(), m_rtc.getMinutes());

	if (loaded_cycle->id !=0) {
...
void Thermostat::toggle_furnace_relay(bool power_ON) {
	if (power_ON != m_furnace_ON) {
		if (power_ON) {
			m_furnace_start_time = m_rtc.getEpoch();
			digitalWrite(FURNACE_RELAY_PIN, HIGH);
		}
		else {
			m_furnace_runtime = m_rtc.getEpoch() - m_furnace_start_time;
